// This Build file is from
// https://github.com/remcohaszing/monaco-tailwindcss/blob/main/build.js
  ...
  plugins: [
    {
      ...
      async setup({ onLoad, onResolve, resolve }) {
        ...
        onResolve(
          { filter: new RegExp(`^(${stubNames.join("|")})$`) },
          ({ path }) => ({
            path: fileURLToPath(
              new URL(`src/stubs/${path}.ts`, import.meta.url)
            ),
          })
        );

        // The tailwindcss main export exports CJS, but we can get better tree shaking if we import
        // from the ESM src directory instead.
        ...
        onResolve(
          { filter: /^tailwindcss-language-service$/ },
          ({ path, ...options }) =>
            resolve(
              "tailwindcss-language-service/dist/tailwindcss-language-service.esm.js",
              options
            )
        );
        ...
        onResolve(
          ...
          ({ path, ...options }) => {
            if (options.importer.includes(`${sep}tailwindcss${sep}`)) {
              return {
                path: fileURLToPath(
                  new URL("src/stubs/tailwindcss/utils/log.ts", import.meta.url)
                ),
              };
            }
            return resolve(path, {
        ...
        onLoad(
          { filter: /\/node_modules\/tailwindcss\/.+\.js$/ },
          async ({ path }) => {
